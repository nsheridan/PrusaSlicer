...
void PresetUpdater::priv::sync_version() const
{
	...
	Http::get(version_check_url)
		...
		.on_complete([&](std::string body, unsigned /* http_status */) {
			...
			// alpha / beta version
			size_t nexn_nl_pos = 0;
			while (body.size() > first_nl_pos + nexn_nl_pos + 1) {
				nexn_nl_pos = body.find_first_of("\n\r", first_nl_pos + nexn_nl_pos + 1);
				if (nexn_nl_pos == std::string::npos)
					break;
				std::string line = body.substr(first_nl_pos + 1, nexn_nl_pos - first_nl_pos - 1);
				// alpha
